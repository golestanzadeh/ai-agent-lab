...
The reviewed work-equipment candidate comprises `330122` (item requires sum), `100200100` (negative item total forbidden), `100200110` (sum requires item), `122050` (sum comparison using `UngleichMitToleranz5`), and `121410` (type and amount together). The local field/rule evidence does not define that tolerance operator, so this candidate is blocked until its exact authoritative semantics are recovered; it must not be approximated.
