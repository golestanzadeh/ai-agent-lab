...
- Bounded local E10/2024 Anlage N mapping profile version 9 is implemented from reviewed official material, including explicit single-item professional-association, work-equipment, home-office workroom, training, home-office-day, and ferry-or-flight subsets. Its declaration validates locally against the exact hash-pinned official `E10-2024.xsd`; official ERiC plausibility execution remains blocked.
- Source-evidenced local plausibility profile version 10 evaluates thirty-six official rules, including paired ferry-or-flight semantics, the combined home-office-day maximum rule, other-expense completeness, and the exact official `UngleichMitToleranz5` boundary.
...
- Execution is `TOKEN_PAUSED`: the current five-hour stop threshold is active, with reset-aligned resume evaluation scheduled for `2026-09-22T03:28:26Z`. Repository recovery remains required after resume conditions are verified.
...
- The readiness and UI presentation contracts are reconciled with the thirty-six-rule local plausibility profile.
...
- Historical package-2 adapter and synthetic-preview records are reconciled with the current E10 readiness pipeline: their immutable upstream status names remain historical and cannot override mapping profile v9, exact official-XSD validation, or the thirty-six-rule local plausibility result.
...
- **Reset-aware pause:** `state=TOKEN_PAUSED`; `limiting_window=five_hour`; `five_hour_remaining_percent=15`; `weekly_remaining_percent=26`; `five_hour_resets_at=1790047586` (`2026-09-22T03:26:26Z`); `weekly_resets_at=1790525245` (`2026-09-27T16:07:25Z`); `resume_not_before=1790047706` (`2026-09-22T03:28:26Z`); `continuation_commit=25c6ad1285fda3b043d896c7daa8c44d0e86e9ba`.
- **Execution-window terminal evaluation:** the five-hour stop threshold requires governed `TOKEN_PAUSED`; explicit terminal condition `(1)` applies. No project package may start before the reset-aligned wake verifies five-hour remaining at least `80%`, weekly remaining above `10%`, and safe repository recovery.
- **Exact next action:** at or after `2026-09-22T03:28:26Z`, read both live limits first; if resume conditions hold, durably end `TOKEN_PAUSED`, restore the five-minute cadence, run the recovery gate from the continuation commit, and assess the next source-evidenced E10 package against its actual size and all existing Human Gates.
