...
- Source-evidenced local plausibility profile version 9 evaluates thirty-five official rules, including the combined home-office-day maximum rule, other-expense completeness, and the exact official `UngleichMitToleranz5` boundary.
...
- The readiness and UI presentation contracts are reconciled with the thirty-five-rule local plausibility profile.
...
- Historical package-2 adapter and synthetic-preview records are reconciled with the current E10 readiness pipeline: their immutable upstream status names remain historical and cannot override mapping profile v8, exact official-XSD validation, or the thirty-five-rule local plausibility result.
...
- **Exact next action:** after pushing plausibility profile v9, read live limits and select the next authorized, prerequisite-ready local synthetic package under the continuous-execution rules.
