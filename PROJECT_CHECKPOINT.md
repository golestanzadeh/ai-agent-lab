...
- The Project Owner authorized local synthetic non-production repair/replay policy design. The policy now defines exact-prefix recovery, one write-once attempt per planned checkpoint, idempotent completed-state handling, duplicate prevention, and fail-closed ambiguity; no mutating repair executor exists yet.
- **Exact next action:** implement the versioned, non-mutating repair-policy evaluator and negative tests described in `docs/agent-runtime-repair-policy.md`; do not connect it to runtime mutation until that bounded evaluator is verified.
