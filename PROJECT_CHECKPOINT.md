...
- Execution is `AGENT_LED_CONTINUOUS_EXECUTION_ACTIVE`: the reset-aware resume check reported five-hour `99%` and weekly `53%` remaining, satisfying the existing resume conditions. Repository recovery remains required before project work.
...
- **Exact next action:** after repository recovery, connect an eligible repair decision to a bounded remaining-stage continuation executor while preserving write-once attempt consumption, exact-prefix recovery, existing acceptance/budget/kill-switch controls, and all external capability denials.
