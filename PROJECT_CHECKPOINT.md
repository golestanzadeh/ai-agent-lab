...
- **Execution-window terminal evaluation:** the post-package observation is five-hour `20%` and weekly `26%` remaining. The five-hour caution rule forbids another large package, and no further source-evidenced, prerequisite-ready independent small package remains. Explicit terminal condition `(3)` therefore applies; successful completion and repository cleanliness are not being treated as terminal conditions.
- **Exact next action:** on the next execution, run the recovery gate and obtain the fallback live observation; begin the next source-evidenced E10 package only if the existing thresholds permit its actual size, otherwise make no project change.
