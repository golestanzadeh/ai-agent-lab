...
- Exact continuation: the Human fills the visible registration form and CAPTCHA without submitting it and supplies action-time registration confirmation; the Human also chooses or authorizes the UI architecture/framework before an interactive prototype. Until one of those inputs arrives, keep the hourly continuation guard paused and make no project change.
